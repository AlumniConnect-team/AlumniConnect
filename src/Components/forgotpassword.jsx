...
const Updatepwd = () => {
    ...
    const handleSendOtp = async () => {
        if (email.includes('@')) {
            ...
            setMessage('');
            try {
                const res = await axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/verify-email", {
                    email,
    ...
    const handleVerifyOtp = async () => {
        setLoading(true);
        try {
            const res = await axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/verify-otp", {
                email,
                otp
            });
            setStep(3);
    ...
    const handleUpdatePassword = async () => {
        if (newPassword === confirmPassword && newPassword !== '') {
            setLoading(true);
            try {
                const res = await axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/changepwd", { 
                    email, 
                    newPassword 
                });
                setMessage(res.data);
