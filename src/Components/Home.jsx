...
const Home = () => {
  ...
  const fetchNetwork = async (query = "") => {
    ...
    try {
      ...
      const res = await API.post(
        import.meta.env.VITE_SERVER_DOMAIN + "/search-alumni",
        payload,
      );
  ...
  useEffect(() => {
    ...
    if (activeTab === "connections") {
      ...
      filtered = filtered.filter(
        (u) =>
          !userConnections.includes(u._id) &&
          parseInt(u.graduationYear) <= parseInt(user.graduationYear),
      );
      ...
      <div className="min-h-screen bg-white font-sans">
        <div className="bg-slate-900 text-white pt-32 pb-24 px-6 text-center rounded-b-[4rem] shadow-2xl relative">
          ...
          <p className="text-xl md:text-2xl text-slate-300 max-w-3xl mx-auto mb-10 leading-relaxed">
            Bridging the gap between students and alumni. Build your network,
            get job referrals, and attend exclusive events hosted by graduates
            from your college.
          </p>
        ...
        <div className="py-20 px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-md hover:-translate-y-2 transition-transform">
            ...
            <p className="text-slate-600 leading-relaxed">
              Find and connect with seniors who have walked your path. Whether
              you need career advice or a mentor, your college community is here
              to help.
            </p>
          </div>
          <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-md hover:-translate-y-2 transition-transform">
            ...
            <p className="text-slate-600 leading-relaxed">
              Access job postings and direct referral opportunities posted
              exclusively by alumni working at top companies around the world.
            </p>
          </div>
          <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-md hover:-translate-y-2 transition-transform">
            <div className="text-5xl mb-6">📅</div>
            ...
            <p className="text-slate-600 leading-relaxed">
              Register for workshops, guest lectures, and networking meetups
              hosted by experienced alumni to boost your skills.
            </p>
          </div>
        </div>
      </div>
  ...
  if (user && !user.isVerified) {
    ...
    const unverifiedList = allUsers.filter(
      (u) => u._id !== user._id && u.isVerified,
    );

    return (
      <div className="min-h-screen bg-gray-50 font-sans pb-12">
        <div className="bg-yellow-50 text-yellow-800 p-4 text-center border-b border-yellow-200">
          <strong>Action Required:</strong> Your account is pending
          verification.
        </div>
        ...
        <div className="max-w-4xl mx-auto pt-12 px-6 text-center">
          ...
          <p className="text-lg text-slate-600 mb-8 max-w-2xl mx-auto">
            To keep AlumniConnect secure and spam-free, we require new alumni to
            be vouched for by <strong>2 verified peers</strong>. Search for your
            batchmates below and ask them to verify you!
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
            {loading ? (
              <p className="text-center col-span-full text-gray-500 font-bold">
                Loading network...
              </p>
            ) : (
                ...
                >
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      user.vouchRequests?.includes(person._id)
                    }
    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-12">
      <div className="bg-slate-900 text-white pt-20 pb-32 px-4 text-center rounded-b-[3rem] shadow-2xl relative">
        <h1 className="text-4xl md:text-6xl font-extrabold mb-4 tracking-tight">
          Hello,{" "}
          <span className="text-blue-400">{user.fullName || user.name}</span>
        </h1>
          ...
          <>
            {displayList.map((person) => {
              ...
              return (
                <div
                  key={person._id}
                  className="bg-white rounded-2xl overflow-hidden shadow-lg hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 flex flex-col group border border-gray-100"
                >
                  <div
                    className={`h-24 relative flex justify-center ${isConnected ? "bg-gradient-to-r from-emerald-500 to-teal-500" : "bg-gradient-to-r from-blue-600 to-indigo-600"}`}
                  >
                  ...
                  <div className="pt-14 pb-6 px-6 text-center flex-grow flex flex-col">
                    ...
                    <p
                      className={`text-xs font-bold uppercase tracking-wide mb-4 ${isConnected ? "text-emerald-600" : "text-blue-600"}`}
                    >
                    ...
                    <div className="space-y-3 text-sm flex-grow">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className="text-slate-800 font-bold truncate max-w-[120px]"
                          title={person.currentCompany}
                        >
                    ...
                    <div className="mt-6 flex gap-3">
                      ...
                      <button
                        onClick={() =>
                          !isConnected &&
                          !isPending &&
                          handleConnectRequest(person._id)
                        }
